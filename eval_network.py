...
tf.app.flags.DEFINE_float(
    'select_threshold', 0.01, 'Selection threshold.')#0.01
tf.app.flags.DEFINE_integer(
    'select_top_k', 400, 'Select top-k detected bounding boxes.')
tf.app.flags.DEFINE_integer(
    'keep_top_k', 400, 'Keep top-k detected objects.')
tf.app.flags.DEFINE_float(
    'nms_threshold', 0.3, 'Non-Maximum Selection threshold.')
tf.app.flags.DEFINE_float(
    'matching_threshold', 0.3, 'Matching threshold with groundtruth objects.')
tf.app.flags.DEFINE_integer(
...
tf.app.flags.DEFINE_string(
    'checkpoint_path', 'tmp/NFS_6_newlabel/NFS_6_5/',#tmp/SSD_of_2_grades_NFS/NFS_6_2/
    'The directory where the model was written to or an absolute path to a '#####
    'checkpoint file.')
tf.app.flags.DEFINE_string(
    'eval_dir', 'tmp/NFS_6_newlabel/NFS_6_5/', 'Directory where the results are saved to.')######
tf.app.flags.DEFINE_integer(
    'num_preprocessing_threads', 4,
    'The number of threads used to create the batches.')
tf.app.flags.DEFINE_string(
    'dataset_name', 'NFS', 'The name of the dataset to load.')
tf.app.flags.DEFINE_string(
    'dataset_split_name', 'test', 'The name of the train/test split.')
tf.app.flags.DEFINE_string(
    'dataset_dir', 'tfrecords/NFS_6_new_5/', 'The directory where the dataset files are stored.')#####
tf.app.flags.DEFINE_string(
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(arg_scope):
            predictions, localisations, logits, end_points, feature1, feature2, feature3, feature4 = \
        ...
        with tf.device('/device:CPU:0'):
            ...
            F1_mean = {}
            mean_precisions = []
            for c in tp_fp_metric[0].keys(): # variable c is class
                prec, rec = tfe.precision_recall(*tp_fp_metric[0][c])
                                                
                precision = tfe.precision_recall_values(LIST_RECALLS, prec, rec)
                mean_precisions.append(precision)
                #np.savetxt('precision_%s.txt' %c, list(precision),fmt='%5s')
                for i, t in enumerate(precision):
                    summary_name = 'precision_at/%s' %c
                    summary_name = summary_name + '/%s' %LIST_RECALLS[i]
                    op = tf.summary.scalar(summary_name, t, collections=[])
                    op = tf.Print(op, [t], summary_name)
                    tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
                #TPR = rec
               # F1=F1
               # summary_name = 'F1/%s' % c
               # op = tf.summary.scalar(summary_name, F1, collections=[])
              #  op = tf.Print(op, [F1], summary_name)
              #  tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
              #  F1_mean[c] = F1
            ...
            #summary_name =  'F1_mean'
            #F1_mean = tf.add_n(list(F1_mean.values())) / len(aps_voc07)
            #op = tf.summary.scalar(summary_name, F1_mean, collections=[])
            #op = tf.Print(op,[F1_mean], summary_name)
            #tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
            ...
            # precision_at_recall
            #c = len(aps_voc07)
            #for c in mean_precisions:
             #   precision += mean_precisions[c]
            # = [c,c,c,c,c,c,c,c]
            #precision = []
            #for i, j in mean_precisions.iteritems():
            #    j = [j]
            #    precision.append(j)
           ...
           # precision=tf.add_n(mean_precisions) / len(aps_voc07)
                
           # precision = precision.eval(session=tf.Session())
            #print precision
            #sess = tf.InteractiveSession()
            #summary_name = 'precision'
            #op = tf.summary.scalar(summary_name, precision, collections=[])
            #op = tf.Print(op, [precision], summary_name)
            #tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
            #precision = tf.div(precision,c)
            #print(tf.shape(precision))
            #np.savetxt('precision.txt',list(precision),fmt='%5s')
            #for i, item in enumerate(list(precision)):
             #   summary_name = 'precision_at/%s' %LIST_RECALLS[i]
             #   op = tf.summary.scalar(summary_name, item, collections=[])
             #   op = tf.Print(op, [item], summary_name)
             #   tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)
            #for i in precision:
            #    summary_name = 'precision_at/'
            #    op = tf.summary.scalar(summary_name, i, collections=[])
            #    op = tf.Print(op, [i], summary_name)
             #   tf.add_to_collection(tf.GraphKeys.SUMMARIES, op)            
            #summary_name = 'mean_presion/%s'%
